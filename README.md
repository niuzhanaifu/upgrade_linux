...
只有服务端实际向设备下发可升级 manifest，或者本应下发但签名/文件异常时，才会记录来源 IP、请求时间、Device-Id、Client-Id、板型、当前版本、目标版本、包名和结果。普通开机检查得到“无升级”不会写入记录，避免用户量大时产生大量冗余数据。该接口会同时返回全局统计和 `version_groups`，前端按目标版本分类展示每个版本的升级记录与统计。
